...
import (
	...
	"github.com/baqupio/baqup/v3/cmd/healthcheck"
	cmdVersion "github.com/baqupio/baqup/v3/cmd/version"
	tcli "github.com/baqupio/baqup/v3/pkg/cli"
	"github.com/baqupio/baqup/v3/pkg/collector"
	...
	"github.com/baqupio/baqup/v3/pkg/provider/tailscale"
	"github.com/baqupio/baqup/v3/pkg/proxy"
	...
	"github.com/baqupio/baqup/v3/pkg/server"
...
func main() {
	...
	cmdBaqup := &cli.Command{
		Name: "baqup",
		Description: `Baqup is a modern HTTP reverse proxy and load balancer made to deploy microservices with ease.
Complete documentation is available at https://baqup.io`,
	...
	err := cmdBaqup.AddCommand(healthcheck.NewCmd(&tConfig.Configuration, loaders))
	if err != nil {
		stdlog.Println(err)
		os.Exit(1)
	}

	err = cmdBaqup.AddCommand(cmdVersion.NewCmd())
	if err != nil {
		stdlog.Println(err)
		os.Exit(1)
	}

	err = cli.Execute(cmdBaqup)
	if err != nil {
...
func runCmd(staticConfiguration *static.Configuration) error {
	...
	log.Info().Str("version", version.Version).
		Msgf("Baqup version %s built on %s", version.Version, version.BuildDate)
...
func setupServer(staticConfiguration *static.Configuration) (*server.Server, error) {
	...
	err := providerAggregator.AddProvider(baqup.New(*staticConfiguration))
	if err != nil {
		return nil, err
	}
	...
	tlsManager := baquptls.NewManager(staticConfiguration.OCSP)
...
func initACMEProvider(c *static.Configuration, providerAggregator *aggregator.ProviderAggregator, tlsManager *baquptls.Manager, httpChallengeProvider, tlsChallengeProvider challenge.Provider, routinesPool *safe.Pool) []*acme.Provider {
...
func checkNewVersion(staticConfiguration *static.Configuration) {
	...
	if staticConfiguration.Global.CheckNewVersion {
		...
		logger.Info().Msg(`Baqup checks for new releases to notify you if your version is out of date.`)
		logger.Info().Msg(`It also collects usage data during this process.`)
		logger.Info().Msg(`Check the documentation to get more info: https://doc.baqup.io/baqup/contributing/data-collection/`)
...
More details: https://doc.baqup.io/baqup/contributing/data-collection/
`)
	}
}
...
func stats(staticConfiguration *static.Configuration) {
	...
	if staticConfiguration.Global.SendAnonymousUsage {
		...
		logger.Info().Msg(`Many thanks for contributing to Baqup's improvement by allowing us to receive anonymous information from your configuration.`)
		logger.Info().Msg(`Help us improve Baqup by leaving this feature on :)`)
		logger.Info().Msg(`More details on: https://doc.baqup.io/baqup/contributing/data-collection/`)
		collect(staticConfiguration)
	} else {
		logger.Info().Msg(`
Stats collection is disabled.
Help us improve Baqup by turning this feature on :)
More details on: https://doc.baqup.io/baqup/contributing/data-collection/
`)
	}
}
