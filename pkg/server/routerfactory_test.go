...
func TestReuseService(t *testing.T) {
	...
	factory := NewRouterFactory(staticConfig, managerFactory, tlsManager, middleware.NewChainBuilder(nil, nil, nil), nil, metrics.NewVoidRegistry(), tcp.NewDialerManager())
...
func TestInternalServices(t *testing.T) {
	...
	factory := NewRouterFactory(staticConfig, managerFactory, tlsManager, middleware.NewChainBuilder(voidRegistry, nil, nil), nil, voidRegistry, tcp.NewDialerManager())
