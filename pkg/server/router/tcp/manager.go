...
import (
	...
	"github.com/baqupio/baqup/v3/pkg/middlewares/snicheck"
	...
	tcpservice "github.com/baqupio/baqup/v3/pkg/server/service/tcp"
...
func (m *Manager) buildEntryPointHandler(ctx context.Context, configs map[string]*runtime.TCPRouterInfo, configsHTTP map[string]*runtime.RouterInfo, handlerHTTP, handlerHTTPS http.Handler) (*Router, error) {
	...
	defaultTLSConf, err := m.tlsManager.Get(baquptls.DefaultTLSStoreName, baquptls.DefaultTLSConfigName)
	if err != nil {
	...
	for routerHTTPName, routerHTTPConfig := range configsHTTP {
		...
		tlsOptionsName := baquptls.DefaultTLSConfigName
		if len(routerHTTPConfig.TLS.Options) > 0 && routerHTTPConfig.TLS.Options != baquptls.DefaultTLSConfigName {
		...
		// we actually rely later on the fact that a tls config is nil (which happens when an error is returned) to take special steps
		// when assigning a handler to a route.
		tlsConf, tlsConfErr := m.tlsManager.Get(baquptls.DefaultTLSStoreName, tlsOptionsName)
		if tlsConfErr != nil {
		...
		for _, domain := range domains {
			...
			if name, ok := tlsOptionsForHost[domain]; ok && name != tlsOptionsName {
				...
				tlsOptionsForHost[domain] = baquptls.DefaultTLSConfigName
			} else {
...
func (m *Manager) addTCPHandlers(ctx context.Context, configs map[string]*runtime.TCPRouterInfo, router *Router) {
	for routerName, routerConfig := range configs {
		...
		if tlsOptionsName != baquptls.DefaultTLSConfigName {
			tlsOptionsName = provider.GetQualifiedName(ctxRouter, tlsOptionsName)
		}

		tlsConf, err := m.tlsManager.Get(baquptls.DefaultTLSStoreName, tlsOptionsName)
		if err != nil {
			routerConfig.AddError(err, true)
			logger.Error().Err(err).Send()
