...
func TestCloseWrite(t *testing.T) {
	...
	require.NoError(t, err)
	proxy, err := NewProxy(":"+port, 10*time.Millisecond, http.DefaultTransport, nil)
	require.NoError(t, err)
...
func TestProxyProtocol(t *testing.T) {
	...
	for _, test := range testCases {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			require.NoError(t, err)
			proxy, err := NewProxy(":"+port, 10*time.Millisecond, http.DefaultTransport, &dynamic.ProxyProtocol{Version: test.version})
			require.NoError(t, err)
...
func TestLookupAddress(t *testing.T) {
	...
	for _, test := range testCases {
		...
		t.Run(test.desc, func(t *testing.T) {
			t.Parallel()
			proxy, err := NewProxy(test.address, 10*time.Millisecond, http.DefaultTransport, nil)
			require.NoError(t, err)
