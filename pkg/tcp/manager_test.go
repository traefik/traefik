...
func TestConflictingConfig(t *testing.T) {
	...
	tlsManager := client.NewTLSConfigManager[*dynamic.TCPServersTransport](nil)
	tlsManager.Update(dynamicConf)

	dialerManager := NewDialerManager(tlsManager)

	dialerManager.Update(dynamicConf)

	_, err := dialerManager.Get("test", true, nil)
	require.Error(t, err)
}
...
func TestNoTLS(t *testing.T) {
	...
	tlsClientManager.Update(dynamicConf)

	dialerManager := NewDialerManager(tlsClientManager)

	dialerManager.Update(dynamicConf)

	dialer, err := dialerManager.Get("test", false, nil)
	require.NoError(t, err)
...
func TestTLS(t *testing.T) {
	...
	dialer, err := dialerManager.Get("test", true, nil)
	require.NoError(t, err)
