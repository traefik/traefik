...
func (b Builder) Build(pName string, config map[string]interface{}, middlewareName string) (Constructor, error) {
	...
	yaegiDescriptor, ok := b.yaegiMiddlewareBuilders[pName]
	if !ok {
		wasmDescriptor, ok := b.wasmMiddlewareBuilders[pName]
		if !ok {
			return nil, fmt.Errorf("unknown plugin type: %s", pName)
		}
		m, err := newWasmMiddleware(wasmDescriptor, config, middlewareName)
		if err != nil {
			return nil, err
		}
...
func newWasmMiddleware(builder *wasmMiddlewareBuilder, config map[string]interface{}, middlewareName string) (*WasmMiddleware, error) {
