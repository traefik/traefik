...
func (c *CertificateData) AppendCertificate(certs map[string]map[string]*CertificateData, storeName string) error {
	...
	if certExists {
		log.Debug().Msg("Skipping addition of certificate for domain(s) %q, to TLS Store %s, as it already exists for this store.", certKey, storeName)
	} else {
		log.Debug().Msg("Adding certificate for domain(s) %s", certKey)
...
func (c *CertificateData) StapleOCSP() error {
	...
	log.Debug().Msg("ocsp response: %v", ocspResp)
