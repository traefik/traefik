...
type OpenTelemetry struct {
	...
	Timeout              types.Duration    `description:"The max waiting time for the backend to process each spans batch." json:"timeout,omitempty" toml:"timeout,omitempty" yaml:"timeout,omitempty" export:"true"`
...
func (o *OpenTelemetry) SetDefaults() {
	...
	o.Timeout = types.Duration(10 * time.Second)
}
...
type GRPC struct {
	...
	ReconnectionPeriod types.Duration `description:"The minimum amount of time between connection attempts to the target endpoint." json:"reconnectionPeriod,omitempty" toml:"reconnectionPeriod,omitempty" yaml:"reconnectionPeriod,omitempty" export:"true"`
...
type Retry struct {
	InitialInterval types.Duration `description:"The time to wait after the first failure before retrying." json:"initialInterval,omitempty" toml:"initialInterval,omitempty" yaml:"initialInterval,omitempty" export:"true"`
	MaxElapsedTime  types.Duration `description:"The maximum amount of time (including retries) spent trying to send a request/batch." json:"maxElapsedTime,omitempty" toml:"maxElapsedTime,omitempty" yaml:"maxElapsedTime,omitempty" export:"true"`
	MaxInterval     types.Duration `description:"The upper bound on backoff interval." json:"maxInterval,omitempty" toml:"maxInterval,omitempty" yaml:"maxInterval,omitempty" export:"true"`
}

// SetDefaults sets the default values.
func (r *Retry) SetDefaults() {
	r.InitialInterval = types.Duration(5 * time.Second)
	r.MaxElapsedTime = types.Duration(time.Minute)
	r.MaxInterval = types.Duration(30 * time.Second)
}
