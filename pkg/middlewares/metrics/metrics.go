...
func (m *metricsMiddleware) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	if m.needsKubernetesLabels {
		...
		if v, ok := core[accesslog.KubernetesIngressNamespace]; ok {
			labels = append(labels, "namespace", v.(string))
			labels = append(labels, "kind", "ingress")
		} else {
			labels = append(labels, "namespace", "none")
			labels = append(labels, "kind", "none")
		}
		if v, ok := core[accesslog.KubernetesIngressName]; ok {
			labels = append(labels, "name", v.(string))
		} else {
			labels = append(labels, "name", "none")
		}
		if v, ok := core[accesslog.KubernetesServiceName]; ok {
			labels = append(labels, "service_name", v.(string))
		} else {
			labels = append(labels, "service_name", "none")
		}
	}
