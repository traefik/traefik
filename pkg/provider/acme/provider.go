...
func (p *Provider) renewCertificates(ctx context.Context, renewPeriod, gracefulPeriod time.Duration) {
	logger := log.FromContext(ctx)
	...
	if len(toRemove) > 0 {
		...
		if err != nil {
			logger.WithError(err).Infof("Error removing ACME certificates: %+v", toRemove)
		}
	}
