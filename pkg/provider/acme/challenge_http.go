...
func (c *ChallengeHTTP) getTokenValue(ctx context.Context, token, domain string) []byte {
	logger := log.FromContext(ctx)
	logger.Debugf("Retrieving the ACME challenge for %q (token %q)...", domain, token)
	...
	operation := func() error {
		...
		if _, ok := c.httpChallenges[token]; !ok {
			return fmt.Errorf("cannot find challenge for token %q (domain %q)", token, domain)
		}

		var ok bool
		result, ok = c.httpChallenges[token][domain]
		if !ok {
			return fmt.Errorf("cannot find challenge for domain %q (token %q)", domain, token)
		}
	...
	if err != nil {
		logger.Errorf("Cannot retrieve the ACME challenge for %q (token %q): %v", domain, token, err)
		return []byte{}
	}
