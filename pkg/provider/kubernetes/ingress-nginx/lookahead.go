...
func splitNegativeLookahead(path string) (keep, exclude string, ok bool) {
	...
	// The assertion guards only the branch it opens. A `|` at the top level of the
	// tail lets another branch match at the same offset without being asserted
	// against, so `keep && !exclude` refuses a path ingress-nginx serves:
	// `/a/((?!b)c|b)` serves `/a/b` through the second branch, while the exclusion
	// arm matches `/a/b` and the negated matcher then rejects the request.
	if hasTopLevelAlternation(path[end+1:]) {
		return "", "", false
	}
...
// hasTopLevelAlternation reports whether expr contains a `|` that is not nested
// inside a group opened within expr. Escapes and character classes are skipped
// the same way matchingParen skips them.
func hasTopLevelAlternation(expr string) bool {
	var depth int
	var inClass bool
	...
	for i := 0; i < len(expr); i++ {
		switch expr[i] {
		case '\\':
			// Skip the escaped byte so that `\|` is not read as an alternation.
			i++
		case '[':
			inClass = true
		case ']':
