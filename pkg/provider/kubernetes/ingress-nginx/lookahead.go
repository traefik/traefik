...
var (
	...
	// literalTail matches the only tail this translation supports: the capture group
	// the lookahead opened closes at the very end of the path with a single `.*`
	// inside it. Anything else — another quantifier, a character class, a trailing
	// literal, a nested group, a top-level alternation, or any expression after the
	// outer capture — is left untranslated, because each of them can change what the
	// path matches in a way `keep && !exclude` does not reproduce.
	literalTail = regexp.MustCompile(`^\.\*\)$`)
)
...
func splitNegativeLookahead(path string) (keep, exclude string, ok bool) {
	...
	// The assertion guards only the branch it opens, so the path has to close out as
	// X((?!A|B|C).*) and nothing more. A top-level alternation lets a branch bypass
	// the assertion — `/a/((?!b)c|b)` still serves `/a/b` through the second branch
	// in ingress-nginx — and an expression after the outer capture changes what
	// matches outright, since `/a/((?!b).*)?b$` makes the guarded group optional.
	if !literalTail.MatchString(path[end+1:]) {
		return "", "", false
	}
...
// resolveNegativeLookahead translates the location path once, before any consumer
// reads it, so the router rule, the rewrite-target regex and the redirect regex
// cannot disagree about whether the path was translatable. Path itself is left as
// ingress-nginx gave it: it stays what an untranslatable path is compiled from, and
// what a translated router's priority is pinned to.
//
// A path the widening step rewrites is never translated. makeTrailingGroupOptional
// turns the literal prefix into a group, and splitNegativeLookahead needs a literal
// prefix so the exclusion arm can anchor at the same determinate offset the assertion
// did. An ImplementationSpecific path carrying both a lookahead and an absolute
// rewrite-target is therefore left verbatim and warned about: that combination is
// unsupported by construction rather than by an explicit guard.
