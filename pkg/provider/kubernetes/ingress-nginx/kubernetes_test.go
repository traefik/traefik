...
func TestLoadIngresses(t *testing.T) {
	testCases := []struct {
		...
		{
			desc: "Upstream keepalive timeout",
			paths: []string{
				"services.yml",
				"ingressclasses.yml",
				"ingresses/ingress-with-upstream-keepalive-timeout.yml",
			},
			expected: &dynamic.Configuration{
				TCP: &dynamic.TCPConfiguration{
					Routers:  map[string]*dynamic.TCPRouter{},
				...
				HTTP: &dynamic.HTTPConfiguration{
					Routers: map[string]*dynamic.Router{
						"default-ingress-with-upstream-keepalive-timeout-rule-0-path-0": {
							Rule:       "Host(`upstream-keepalive.localhost`) && Path(`/`)",
							RuleSyntax: "default",
							Service:    "default-ingress-with-upstream-keepalive-timeout-whoami-80",
						},
					},
					Middlewares: map[string]*dynamic.Middleware{},
					Services: map[string]*dynamic.Service{
						"default-ingress-with-upstream-keepalive-timeout-whoami-80": {
							LoadBalancer: &dynamic.ServersLoadBalancer{
								Servers: []dynamic.Server{
									{URL: "http://10.10.0.1:80"},
									{URL: "http://10.10.0.2:80"},
								...
								ServersTransport: "default-ingress-with-upstream-keepalive-timeout",
							},
						},
					},
					ServersTransports: map[string]*dynamic.ServersTransport{
						"default-ingress-with-upstream-keepalive-timeout": {
							ForwardingTimeouts: &dynamic.ForwardingTimeouts{
								DialTimeout:     ptypes.Duration(60 * time.Second),
								IdleConnTimeout: ptypes.Duration(120 * time.Second), // The parsed annotation value
							},
						},
					},
				},
