...
func createCircuitBreakerMiddleware(circuitBreaker *v1.CircuitBreaker) (*dynamic.CircuitBreaker, error) {
...
func createRateLimitMiddleware(rateLimit *v1.RateLimit) (*dynamic.RateLimit, error) {
	if rateLimit == nil {
		return nil, nil
	}
...
func createRetryMiddleware(retry *v1.Retry) (*dynamic.Retry, error) {
	if retry == nil {
		return nil, nil
	}
...
func (p *Provider) createErrorPageMiddleware(client Client, namespace string, errorPage *v1.ErrorPage) (*dynamic.ErrorPage, *dynamic.Service, error) {
	if errorPage == nil {
		return nil, nil, nil
	}
...
func createForwardAuthMiddleware(k8sClient Client, namespace string, auth *v1.ForwardAuth) (*dynamic.ForwardAuth, error) {
	if auth == nil {
		return nil, nil
	}
...
func createBasicAuthMiddleware(client Client, namespace string, basicAuth *v1.BasicAuth) (*dynamic.BasicAuth, error) {
	if basicAuth == nil {
		return nil, nil
	}
...
func createDigestAuthMiddleware(client Client, namespace string, digestAuth *v1.DigestAuth) (*dynamic.DigestAuth, error) {
	if digestAuth == nil {
		return nil, nil
	}
...
func createChainMiddleware(ctx context.Context, namespace string, chain *v1.Chain) *dynamic.Chain {
	if chain == nil {
		return nil
	}
...
func buildCertificates(client Client, tlsStore, namespace string, certificates []v1.Certificate, tlsConfigs map[string]*tls.CertAndStores) error {
