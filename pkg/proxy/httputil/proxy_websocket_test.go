...
func TestWebSocketRequestWithHeadersInResponseWriter(t *testing.T) {
	...
	p, err := NewProxyBuilder(transportManager, nil, false).Build("default@internal", testhelpers.MustParseURL(srv.URL), true, false, 0)
	require.NoError(t, err)
...
func createProxyWithForwarder(t *testing.T, uri string, transport http.RoundTripper) *httptest.Server {
	...
	p, err := NewProxyBuilder(transportManager, nil, false).Build("fwd", u, true, false, 0)
	require.NoError(t, err)
