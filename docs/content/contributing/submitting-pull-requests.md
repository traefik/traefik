...
If you're looking for information on setting up your developer environment 
...
Unfortunately, 
our team has to prioritize pull requests (PRs) for review 
(but we are welcoming new [maintainers](https://github.com/traefik/traefik/blob/master/docs/content/contributing/maintainers-guidelines.md) to speed this up, so if you are interested, check it out and apply). 
...
If you have questions about the Triage process, [read more here](https://github.com/traefik/contributors-guide/blob/master/issue_triage.md).  
...
!!! note
    ...
    Occasionally, 
    we may freeze our codebase when working towards a specific feature or goal that could impact other development. 
    During this time,
    your pull request could remain unmerged while the release work is completed.
...
In addition,
it rebases or merges with the base PR branch if needed. 
...
By default, 
a squash-rebase merge will be carried out.

The status `status/4-merge-in-progress` is only used by the bot.

If the bot is not able to perform the merge, 
the label `bot/need-human-merge` is added. 
In such a situation,
solve the conflicts/CI/...
and then remove the label bot/need-human-merge.

To prevent the bot from automatically merging a PR,
add the label `bot/no-merge`.
...
Sometimes,
this means that we will not be able to merge a pull request.

Because we respect the work you did,
you will always be told why we are closing your pull request. 
If you do not agree with our decision, don’t worry; 
closed pull requests are easy to recreate, 
...
We put our top priorities on our roadmap 
and you can identify them by the `contributor/wanted` tag. 
...
Additionally,
during the last few weeks of a milestone,
we stop reviewing PRs to reduce churn and stabilize. 
...
* You didn’t create an issue for the PR you wish to make. 
  If you do not create an issue before submitting your PR, 
  we will not be able to answer any design questions
  and let you know how likely your PR is to be merged. 
...
* Your tests were inadequate or absent
    * If you don't know how to test your PR, 
      please ask! We'll be happy to help you or suggest appropriate test cases

If you've already followed the best practices and your PR still has not received a response, 
...
In this situation, 
you can ask the primary reviewer (assignee) whether they want you to fork a new pull request to clear out all the comments. 
