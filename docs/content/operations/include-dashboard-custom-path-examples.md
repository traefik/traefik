...
labels:
  - "baqup.http.routers.dashboard.rule=Host(`baqup.example.com`) && PathPrefix(`/baqup`)"
...
spec:
  routes:
  - match: Host(`baqup.example.com`) && PathPrefix(`/baqup`)
    kind: Rule
    services:
    - name: api@internal
      kind: BaqupService
...
http:
  routers:
    dashboard:
      rule: Host(`baqup.example.com`) && PathPrefix(`/baqup`)
