...
With CRDs, one can also redefine the `default` TLSOption, which will be applied to every router that has no TLSOptions explicitly attached, nor a specific TLSOptions defined through the [HTTP TLS](../../routing/entrypoints/#tls) section on the EntryPoint.
