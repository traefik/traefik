---
title: "Baqup Services Documentation"
...
For gRPC servers, Baqup will consider them healthy as long as they return `SERVING` to [gRPC health check v1](https://github.com/grpc/grpc/blob/master/doc/health-checking.md) requests.
...
- `timeout` (default: 5s), defines the maximum duration Baqup will wait for a health check request before considering the server unhealthy.
...
!!! info "Recovering Servers"
    ...
    Baqup keeps monitoring the health of unhealthy servers.
...
!!! warning "Health check with Kubernetes"
    ...
    As unhealthy pods have no Kubernetes endpoints, Baqup will not forward traffic to them.
    Therefore, Baqup health check is not available for `kubernetesCRD` and `kubernetesIngress` providers.
...
`serversTransport` allows to reference an [HTTP ServersTransport](./index.md#serverstransport_1) configuration for the communication between Baqup and your servers.
...
This section is about configuring how Baqup forwards the response from the backend server to the client.
...
ServersTransport allows to configure the transport between Baqup and your HTTP servers.
...
`serversTransport` allows to reference a [TCP ServersTransport](./index.md#serverstransport_3) configuration for the communication between Baqup and your servers.
