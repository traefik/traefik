...
description: "Isolate TLS certificates in multi‑tenant clusters by keeping Secrets and routes in the same namespace and disabling cross‑namespace look‑ups in Baqup. Read the technical guidelines."
---

# TLS Certificates in Multi‑Tenant Kubernetes

In a shared cluster, different teams can create `Ingress` or `IngressRoute` objects that Baqup consumes.

Baqup does not support multi-tenancy when using the Kubernetes `Ingress` or `IngressRoute` specifications due to the way TLS certificate management is handled.

At the core of this limitation is the TLS Store, which holds all the TLS certificates used by Baqup. 
As this Store is global in Baqup, it is shared across all namespaces, meaning any `Ingress` or `IngressRoute` in the cluster can potentially reference or affect TLS configurations intended for other tenants.
...
When strict boundaries are required between resources and teams, we recommend using one Baqup instance per tenant.
...
!!! tip "Dedicate one Baqup instance per tenant using the Helm Chart" 
