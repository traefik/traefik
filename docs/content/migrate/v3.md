...
The new `peerCertSubjectAltNames` option replaces it and supports multiple Subject Alternative Names (SANs) of type `URI` or `DNSName`.

Please check out the [ServersTransport](../reference/routing-configuration/http/load-balancing/serverstransport.md#opt-peerCertSubjectAltNames) and [ServersTransportTCP](../reference/routing-configuration/tcp/serverstransport.md#opt-serverstransport-tls-peerCertSubjectAltNames) documentation for more details.

#### Kubernetes CRD Provider

To use the new `peerCertSubjectAltNames` field on `ServersTransport` and `ServersTransportTCP` resources with the Kubernetes CRD provider, you need to update your CRDs.
