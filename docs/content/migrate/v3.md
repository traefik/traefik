...
### Kubernetes Gateway API Provider: generated router, middleware, and weighted service names
...
Middleware names and the generated weighted round robin service name are derived from the router name, so they change accordingly, for example `<router name>-requestheadermodifier-0` and `<router name>-wrr`.
...
!!! warning "Observability"
    ...
    Dashboards, alerting rules, and log queries that match on Gateway API router, middleware, or weighted service names must be updated accordingly.
