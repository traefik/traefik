...
### Kubernetes Ingress-NGINX: `Ssl-Client-*` request headers

An Ingress enabling the `nginx.ingress.kubernetes.io/auth-tls-pass-certificate-to-upstream` annotation
had the `Ssl-Client-*` request headers handled by Traefik on its HTTPS router only.
Starting with `v3.7.14`, they are handled on its HTTP router as well.

A request served by that HTTP router, for instance because `nginx.ingress.kubernetes.io/ssl-redirect` is `false`,
now carries none of them, as Nginx does not set them outside of a TLS connection,
where it previously reached the backend with the values the client sent for them.

The header names aliasing those four, such as `Ssl_Client_Verify`, are removed as well, on both routers.

Please check out the [Headers with Aliasing Names](../security/header-aliases.md) documentation for more details.
