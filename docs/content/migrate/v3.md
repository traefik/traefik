...
In the case where it is prefered that traefik does not have access to Node resources, this lookup can be disabled. This also disabled NodePort load-balancing. When the node lookup is disabled the above mentioned RBAC permissions are not needed anymore.
