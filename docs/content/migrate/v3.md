...
Starting with `v3.8.0`, the new [`tap` middleware](../reference/routing-configuration/http/middlewares/tap.md) sends a copy of the requests and the responses going through it to Traefik services, as JSON records.

Existing configurations are not affected: the middleware only applies to the routers referencing it.

#### Kubernetes CRD Provider

To use the new `tap` middleware with the Kubernetes CRD provider, you need to update your CRDs.
