---
title: "Baqup Migration Documentation"
description: "Learn the steps needed to migrate to new Baqup Proxy v3 versions. Read the technical documentation."
---

# Migration: Steps needed between the versions

This guide provides detailed migration steps for upgrading between different Baqup v3 versions. Each section covers breaking changes, deprecations, and configuration updates required for a smooth transition.
...
Starting with v3.1, Baqup's Kubernetes Providers use the [EndpointSlices API](https://kubernetes.io/docs/concepts/services-networking/endpoint-slices/) (requires Kubernetes >=v1.21) for service endpoint discovery. This change also introduces NodePort load-balancing capabilities.
...
```shell
kubectl apply -f https://raw.githubusercontent.com/baqup/baqup/v3.3/docs/content/reference/dynamic-configuration/kubernetes-crd-definition-v1.yml
```

**Updated Resources:**

- [BaqupService](../routing/services/index.md#mirroring-service) ([PR #11032](https://github.com/baqupio/baqup/pull/11032))
- [RateLimit](../middlewares/http/ratelimit.md) & [InFlightReq](../middlewares/http/inflightreq.md) middlewares ([PR #9747](https://github.com/baqupio/baqup/pull/9747))
- [Compress](../middlewares/http/compress.md) middleware ([PR #10943](https://github.com/baqupio/baqup/pull/10943))
...
Due to breaking changes in Kubernetes Gateway [v1.2.0-rc1](https://github.com/kubernetes-sigs/gateway-api/releases/tag/v1.2.0-rc1), Baqup v3.3 only supports Kubernetes Gateway v1.2.x when experimental features are enabled.
...
In v3.2.1, the `X-Forwarded-Prefix` header is now handled like other `X-Forwarded-*` headers - Baqup removes it when sent from untrusted sources.
...
|------------------|-----------|
| `baqup.docker.network` | `baqup.swarm.network` |
| `baqup.docker.lbswarm` | `baqup.swarm.lbswarm` |
...
- **Metric:** `baqup_(entrypoint|router|service)_request_duration_seconds`
...
```shell
kubectl apply -f https://raw.githubusercontent.com/baqup/baqup/v3.4/docs/content/reference/dynamic-configuration/kubernetes-crd-definition-v1.yml
```
...
# Update RBACs
kubectl apply -f https://raw.githubusercontent.com/baqup/baqup/v3.4/docs/content/reference/dynamic-configuration/kubernetes-crd-rbac.yml
```
...
| Request Path      | Router Rule                  | Baqup v3.4.0 | Baqup v3.4.1 | Explanation                                           |
|-------------------|------------------------------|----------------|----------------|-------------------------------------------------------|
...
It appears that enabling MPTCP on some platforms can cause Baqup to stop with the following error logs message:
...
For that purpose, the following right has to be added to the Baqup Kubernetes RBACs:
...
```shell
kubectl apply -f https://raw.githubusercontent.com/baqup/baqup/v3.5/docs/content/reference/dynamic-configuration/kubernetes-crd-definition-v1.yml
```
...
Starting with `v3.5.4`, and when using OpenTelemetry, the `baqup_tls_certs_not_after_milliseconds` metric is renamed to `baqup_tls_certs_not_after_seconds`.
