...
`TCPRoute` and `UDPRoute` have graduated to the Standard channel and are consumed through their `v1` versions, so they do not require the `experimentalChannel` option.
...
Existing `TCPRoute` and `UDPRoute` objects are preserved and served through their `v1` versions once the CRDs are updated.
...
If the cluster uses the Experimental channel for other Gateway API resources, apply the Experimental bundle instead.
Traefik itself no longer requires it:
