...
`TCPRoute` has graduated to the Standard channel (GA) in Gateway API `v1.6.0`.
Traefik `v3.7` keeps consuming the `v1alpha2` version of `TCPRoute`,
and the `experimentalChannel` option is still required to enable it.

`v1alpha2` version of `TCPRoute` is only served with the experimental channel CRDs in Gateway API `v1.6`,
so if you are using `TCPRoute` in your cluster,
and consider using the `v1.6.1` version of the Gateway API CRDs,
you need to install experomental channel CRDs in your cluster.
