...
The following RBAC updates are required for the KubernetesGateway Provider:
...
The KubernetesGateway Provider now needs to lookup Nodes in the cluster to facilitate NodePort load-balancing capabilities.

**New Configuration options:**

In the case where it is preferred that traefik does not have access to Node resources, this lookup can be disabled. This also disabled NodePort load-balancing. When the node lookup is disabled the above mentioned RBAC permissions are not needed anymore.

??? example "Disable kubernetesgateway node resources lookup option"
