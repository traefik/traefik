...
### Adding Root CAs through config maps
v3.4 allows you to add Root CAs through config maps and secrets using the `rootCAs` option in the `ServersTransport` CRD. This also comes with the deprecation
of the `rootCAsSecrets` option. So ServerTransports CRD from v3.3 to v3.4 will look like:
