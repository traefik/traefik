...
### EntryPoints: `allowHeadersWithUnderscores`

In `v2.11.48`, a new `allowHeadersWithUnderscores` option has been added to the entry points HTTP configuration.

By default, the behavior is unchanged: request headers with underscores in their names are forwarded as is.
When set to `false`, such headers are removed from the request before routing.

This prevents spoofing of managed headers (e.g. ForwardAuth `authResponseHeaders`) through their underscore variants,
when backends map both forms to the same variable (CGI, WSGI, PHP, ...).
Setting the option to `false` is recommended when such backends are exposed.

Please check out the [entry points](../routing/entrypoints.md#allowheaderswithunderscores) documentation for more details.
