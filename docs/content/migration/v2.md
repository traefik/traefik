...
|-------------------|-----------------------|-----------------------------------------------------------|
| `%2f` or `%2F`    | `/` (slash)           | `entryPoints.<name>.http.allowEncodedSlash: true`         |
| `%5c` or `%5C`    | `\` (backslash)       | `entryPoints.<name>.http.allowEncodedBackSlash: true`     |
| `%00`             | NULL (null character) | `entryPoints.<name>.http.allowEncodedNullCharacter: true` |
| `%3b` or `%3B`    | `;` (semicolon)       | `entryPoints.<name>.http.allowEncodedSemicolon: true`     |
| `%25`             | `%` (percent)         | `entryPoints.<name>.http.allowEncodedPercent: true`       |
| `%3f` or `%3F`    | `?` (question mark)   | `entryPoints.<name>.http.allowEncodedQuestionMark: true`  |
| `%23`             | `#` (hash)            | `entryPoints.<name>.http.allowEncodedHash: true`          |
