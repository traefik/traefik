...
### `trustForwardHeader` on ForwardAuth middleware

In `v2.11.43`, the `trustForwardHeader` option of the ForwardAuth middleware has been deprecated.

When `trustForwardHeader` is not explicitly set, Traefik logs a warning because the behavior is inconsistent:
...
To silence the warning, explicitly set `trustForwardHeader` to `true` or `false` in your ForwardAuth middleware configuration.
