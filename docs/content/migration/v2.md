...
The deprecated, legacy behavior of treating the CommonName field on X.509 certificates as a host name when no Subject Alternative Names are present, is disabled.
...
As the `x509ignoreCN=0` value for the `GODEBUG` environment variable has been deprecated as well in Go 1.17,
there is no way to restore the old behavior.

More information: https://golang.org/doc/go1.15#commonname and https://tip.golang.org/pkg/crypto/x509/ .
