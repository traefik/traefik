...
Starting with `v2.11.1` the entryPoints `readTimeout` option default value changed to 5 seconds.
...
This change has been done to avoid Traefik instances with default configuration to be hanging forever while waiting for bytes to be read on the connection.
...
Increasing the `readTimout` value could be the solution notably if you are dealing with the following errors:
