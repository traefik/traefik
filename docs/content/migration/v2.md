...
Please note that the unknown fields will not be pruned when migrating from `apiextensions.k8s.io/v1` to `apiextensions.k8s.io/v1` CRDs.
...
Traefik now supports only v1.14+ Kubernetes clusters, which means the support of `extensions/v1` API Version ingresses has been dropped.

The `extensions/v1` API Version should now be replaced either by `networking.k8s.io/v1` or by `networking.k8s.io/v1` (as of Kubernetes v1.19+).

The support of the `networking.k8s.io/v1` API Version will stop in Kubernetes v1.22.
