...
Starting with `v2.11.53`, it can now be configured on the `Middleware` CRD.
...
Starting with `v2.11.53`, HTTP/1 `CONNECT` requests are rejected with a `501 Not Implemented` response.

HTTP/1 `CONNECT` requests were not functional before this change, so this rejection makes it explicit.

## v2.11.55

### Kubernetes Gateway API Provider: generated router and service names

Starting with `v2.11.55`, the Kubernetes Gateway API provider quotes the values it takes from
`HTTPRoute`, `TCPRoute` and `TLSRoute` resources when it builds router rules,
so that a value can no longer alter the rule it is embedded in.

Rules are now built with double-quoted matcher arguments instead of backquoted ones:

```text
Host(`foo.com`) && Path(`/bar`)    ->    Host("foo.com") && Path("/bar")
```

The generated router and service names embed a hash of the rule, so they change as well.
For example, a route previously exposed as
`default-http-app-1-my-gateway-web-1c0cf64bde37d9d0df06@kubernetesgateway` is now exposed as
`default-http-app-1-my-gateway-web-af329269dd38031b03e3@kubernetesgateway`.
...
Starting with `v2.11.55`, the Kubernetes CRD provider derives the names it generates from the identity of the
...
Starting with `v2.11.55`, the new `defaultTLSResourcesNamespace` provider option reserves these resources
...
Please check out the [Kubernetes CRD provider](../providers/kubernetes-crd.md#defaulttlsresourcesnamespace) documentation for more details.
