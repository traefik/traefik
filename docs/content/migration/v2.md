...
This option is deprecated since `v2.11.56`, please use the [aliasHeadersStrategy option](../routing/entrypoints.md#aliasheadersstrategy) instead.
...
Starting with `v2.11.55`, the new `safeNaming` provider option enables collision-safe naming for the routers,
...
Please check out the [`safeNaming`](../providers/kubernetes-crd.md#safenaming) provider documentation for more details.
...
From version `v2.11.56` onwards, a new `aliasHeadersStrategy` entry point option deprecates and replaces the
`underscoreHeadersStrategy` option introduced in `v2.11.51`.
...
Configuring both options with different values makes the static configuration invalid.
