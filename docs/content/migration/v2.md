...
In `v2.11.45`, a new `crossProviderNamespaces` option is available on the Kubernetes CRD, Ingress, and Gateway providers.

Traefik offers the possibility to make references from one resource to another.

This is often used to reference routing resources from one provider to another (cross-provider references), 
but this mechanism also works from within one provider.

However, in the context of Kubernetes providers,
those direct references (e.g.: `myservice@kubernetescrd`) allow a user to cross namespace boundaries, 
as well as exposing `@internal` services, that only the operator should be able to expose.

This new `crossProviderNamespaces` option restricts in which namespaces Kubernetes resources are allowed to use direct references.

The behavior is as follows:

| Value      | Behavior                                                                                         |
|------------|--------------------------------------------------------------------------------------------------|
| not set    | All Kubernetes resources can declare references to Traefik resources.                            |
| `[]`       | Every Kubernetes resource declaring a reference to a Traefik resource is rejected.               |
| `["ns-a"]` | Only Kubernetes resources in the listed namespaces can declare references to a Traefik resource. |

Operators running multi-tenant clusters are encouraged to set this option to avoid privilege escalation across namespace boundaries.
