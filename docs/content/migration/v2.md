...
The `Ingress` `apiVersion` `networking.k8s.io/v1` is now fully supported, and the support of `extensions/v1beta1` has been dropped.

So, prefer using the `networking.k8s.io/v1` as soon as your kubernetes version allows it (v1.19+).

For more details on `Ingress` and `IngressClass` in v1.19+, please check the [API changes](https://v1-19.docs.kubernetes.io/docs/setup/release/notes/#api-change).
