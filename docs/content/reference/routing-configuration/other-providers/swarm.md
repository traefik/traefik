---
title: "Baqup Docker Swarm Routing Documentation"
description: "This guide will teach you how to attach labels to your containers, to route traffic and load balance with Baqup and Docker Swarm."
---

# Baqup & Docker Swarm

One of the best feature of Baqup is to delegate the routing configuration to the application level.
With Docker Swarm, Baqup can leverage labels attached to a service to generate routing rules.
...
??? example "Specify a Custom Port for the Container"
    ...
    services:
      my-container:
        ...
        deploy:
          labels:
            - baqup.http.routers.my-container.rule=Host(`example.com`)
            - baqup.http.routers.my-container.service=my-service"
    ...
    !!! important "Baqup Connecting to the Wrong Port: `HTTP/502 Gateway Error`"
        By default, Baqup uses the lowest exposed port of a container as detailed in
...
Baqup creates, for each container, a corresponding [service](../http/load-balancing/service.md) and [router](../http/routing/rules-and-priority.md).
...
??? info "`baqup.http.services.<service_name>.loadbalancer.server.url`"
    ...
    ```yaml
    baqup.http.services.<service_name>.loadbalancer.server.url=http://foobar:8080
    ```
...
If you enable this option, Baqup will use the virtual IP provided by docker swarm instead of the containers IPs.
Which means that Baqup will not perform any kind of load balancing and will delegate this task to swarm.
