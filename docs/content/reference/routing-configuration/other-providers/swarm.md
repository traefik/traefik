...
| <a id="opt-traefik-http-routers-router-name-constraint" href="#opt-traefik-http-routers-router-name-constraint" title="#opt-traefik-http-routers-router-name-constraint">`traefik.http.routers.<router_name>.constraint`</a> | Constraint expression evaluated against the provider's `routerLabels` static-config map. Router is dropped if expression does not match. Syntax matches `constraints` (`Label`, `LabelRegex`, `&&`, `\|\|`, `!`). | ```Label(`tier`,`ext`)``` |
