...
At high request rates, `h2c` can also reduce connection overhead compared to `http`: a request burst to the same authentication server opens additional short-lived connections, whereas HTTP/2 multiplexes many concurrent requests over a single connection.
