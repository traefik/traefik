...
| <a id="opt-nginx-ingress-kubernetes-iocors-allow-origin" href="#opt-nginx-ingress-kubernetes-iocors-allow-origin" title="#opt-nginx-ingress-kubernetes-iocors-allow-origin">`nginx.ingress.kubernetes.io/cors-allow-origin`</a> | Single-level wildcard origins are supported in the `protocol://*.domain[:port]` form, where `*` matches exactly one DNS label. Wildcards in any other position are ignored, as with ingress-nginx. |
...
| <a id="opt-nginx-ingress-kubernetes-iolimit-allowlist" href="#opt-nginx-ingress-kubernetes-iolimit-allowlist" title="#opt-nginx-ingress-kubernetes-iolimit-allowlist">`nginx.ingress.kubernetes.io/limit-allowlist`</a> | Comma-separated list of IPs and CIDR ranges exempted from `limit-rps`, `limit-rpm` and `limit-connections`. Each router of the Ingress path gets a companion router matching those client IPs, without the limit middlewares: the exempted traffic is reported under that router name in the metrics. Client IPs are matched on the connection remote address, as NGINX does with `geo $remote_addr`. The companion router takes the priority of the router it exempts plus one, so the exemption does not affect the other routers of the host. |
| <a id="opt-nginx-ingress-kubernetes-iolimit-whitelist" href="#opt-nginx-ingress-kubernetes-iolimit-whitelist" title="#opt-nginx-ingress-kubernetes-iolimit-whitelist">`nginx.ingress.kubernetes.io/limit-whitelist`</a> | Legacy alias for `limit-allowlist`, which takes precedence when both are set. |
...
| Annotation                                                                                                                                                                                                                                      | Limitations / Notes                                                                                                                                                                                                                                                                                                                  |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| <a id="opt-nginx-ingress-kubernetes-ioenable-access-log" href="#opt-nginx-ingress-kubernetes-ioenable-access-log" title="#opt-nginx-ingress-kubernetes-ioenable-access-log">`nginx.ingress.kubernetes.io/enable-access-log`</a> | Access logs must first be enabled in the [install configuration](../../../install-configuration/observability/logs-and-accesslogs/#accesslogs) (globally or per entrypoint) for this annotation to take effect. When access logs are enabled, this annotation allows opting out specific Ingresses by setting it to `"false"`. Conversely, when access logs are disabled on an entrypoint, setting this annotation to `"true"` allows opting in specific Ingresses. |
