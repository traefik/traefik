...
| <a id="opt-nginx-ingress-kubernetes-iocors-allow-origin" href="#opt-nginx-ingress-kubernetes-iocors-allow-origin" title="#opt-nginx-ingress-kubernetes-iocors-allow-origin">`nginx.ingress.kubernetes.io/cors-allow-origin`</a> | Single-level wildcard origins are supported in the `protocol://*.domain[:port]` form, where `*` matches exactly one DNS label. Wildcards in any other position are ignored, as with NGINX. |
