...
| <a id="opt-nginx-ingress-kubernetes-iorewrite-target" href="#opt-nginx-ingress-kubernetes-iorewrite-target" title="#opt-nginx-ingress-kubernetes-iorewrite-target">`nginx.ingress.kubernetes.io/rewrite-target`</a> | When combined with `auth-url`/`auth-snippet` on the same Ingress, the ForwardAuth request receives the original request path while the upstream request uses the rewritten path. |
