...
| <a id="opt-nginx-ingress-kubernetes-ioupstream-keepalive-timeout" href="#opt-nginx-ingress-kubernetes-ioupstream-keepalive-timeout" title="#opt-nginx-ingress-kubernetes-ioupstream-keepalive-timeout">`nginx.ingress.kubernetes.io/upstream-keepalive-timeout`</a> | Configures the maximum time (in seconds) an idle connection to the upstream is kept open. Maps to Traefik's `ServersTransport.ForwardingTimeouts.IdleConnTimeout`. |
