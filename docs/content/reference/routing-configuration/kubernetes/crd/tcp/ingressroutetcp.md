...
| <a id="opt-routesn-servicesn-strategy" href="#opt-routesn-servicesn-strategy" title="#opt-routesn-servicesn-strategy">`routes[n].services[n].strategy`</a> | Defines the load balancing strategy. Supported values are `wrr` (Weighted Round Robin) and `ip-hash` (routes connections from the same source IP consistently to the same server). | `wrr` | No |
