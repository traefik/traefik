...
| <a id="opt-serverstransport-forwardingTimeouts-writeTimeout" href="#opt-serverstransport-forwardingTimeouts-writeTimeout" title="#opt-serverstransport-forwardingTimeouts-writeTimeout">`serverstransport.`<br />`forwardingTimeouts.writeTimeout`</a> | Defines a timeout for reading a response from the proxied server. The timeout between two successive read operations. The connection is closed if nothing is transmitted within this time.<br />Zero means no timeout | 30s | No |
