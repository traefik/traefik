---
title: "Baqup Kubernetes Services Documentation"
description: "Learn how to configure routing and load balancing in Baqup Proxy to reach Services, which handle incoming requests. Read the technical documentation."
--- 

A `BaqupService` is a custom resource that sits on top of the Kubernetes Services. It enables advanced load-balancing features such as a [Weighted Round Robin](#weighted-round-robin) load balancing, a [Highest Random Weight](#highest-random-weight) load balancing, or a [Mirroring](#mirroring) between your Kubernetes Services.

Services configure how to reach the actual endpoints that will eventually handle incoming requests. In Baqup, the target service can be either a standard [Kubernetes service](https://kubernetes.io/docs/concepts/services-networking/service/)—which exposes a pod—or a BaqupService. The latter allows you to combine advanced load-balancing options like:
...
The WRR is able to load balance the requests between multiple services based on weights. The WRR `BaqupService` allows you to load balance the traffic between Kubernetes Services and other instances of `BaqupService` (another WRR service -, or a mirroring service).
...
```yaml tab="BaqupService WRR Level#1"
apiVersion: baqup.io/v1alpha1
kind: BaqupService
metadata:
  name: wrr1
...
```yaml tab="BaqupService WRR Level#2"
apiVersion: baqup.io/v1alpha1
kind: BaqupService
metadata:
  name: wrr2
...
|:---------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------|:---------|
| <a id="opt-services" href="#opt-services" title="#opt-services">`services`</a> | List of any combination of BaqupService and [Kubernetes service](https://kubernetes.io/docs/concepts/services-networking/service/). <br />. Exhaustive list of option in the [`Service`](./service.md#configuration-options) documentation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |                                                                      | No       |
...
```yaml tab="BaqupService WRR with 2 level of stickiness"
apiVersion: baqup.io/v1alpha1
kind: BaqupService
metadata:
  name: wrr1
...
| Field                                                          | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Default                                                              | Required |
|:---------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------|:---------|
| <a id="opt-services-2" href="#opt-services-2" title="#opt-services-2">`services`</a> | List of any combination of BaqupService and [Kubernetes service](https://kubernetes.io/docs/concepts/services-networking/service/). Each service must have a weight assigned.                                                                                                                                                                                                                                                                                                                                                                                                                                     |                                                                      | Yes      |
| <a id="opt-servicesm-kind" href="#opt-servicesm-kind" title="#opt-servicesm-kind">`services[m].`<br />`kind`</a> | Kind of the service targeted.<br />Two values allowed:<br />- **Service**: Kubernetes Service<br /> - **BaqupService**: Baqup Service.                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | ""                                                                   | No       |
...
| <a id="opt-servicesm-serversTransport" href="#opt-servicesm-serversTransport" title="#opt-servicesm-serversTransport">`services[m].`<br />`serversTransport`</a> | Name of ServersTransport resource to use to configure the transport between Baqup and your servers.<br />Evaluated only if the kind is **Service**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                | ""                                                                   | No       |
...
| <a id="opt-servicesm-nativeLB" href="#opt-servicesm-nativeLB" title="#opt-servicesm-nativeLB">`services[m].`<br />`nativeLB`</a> | Allow using the Kubernetes Service load balancing between the pods instead of the one provided by Baqup.<br />Evaluated only if the kind is **Service**.                                                                                                                                                                                                                                                                                                                                                                                                                                                           | false                                                                | No       |
...
The mirroring `BaqupService` allows you to reference Kubernetes Services and other instances of `BaqupService` (another WRR service -, or a mirroring service).
...
```yaml tab="Mirroring from a BaqupService (WRR)"
...
| <a id="opt-mirrors" href="#opt-mirrors" title="#opt-mirrors">`mirrors`</a> | List of mirrored services to target.<br /> It can be any combination of BaqupService and [Kubernetes service](https://kubernetes.io/docs/concepts/services-networking/service/). <br />Exhaustive list of option in the [`Service`](./service.md#configuration-options) documentation.                                                                                                                                                                                                                                                                                                                                                      |                                                                   | Yes       |
