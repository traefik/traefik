...
| <a id="opt-echKeys" href="#opt-echKeys" title="#opt-echKeys">`echKeys`</a> | Enable server-side Encrypted Client Hello (ECH).<br />List of names of the referenced Kubernetes [Secrets](https://kubernetes.io/docs/concepts/configuration/secret/) (in TLSOption namespace).<br />The secret must contain an [RFC 9934](https://www.rfc-editor.org/rfc/rfc9934.html) PEM-encoded ECH key under a `tls.ech` key.<br />See [Encrypted Client Hello keys](../../../http/tls/tls-options.md#encrypted-client-hello-keys) for more information.                              |                            | No       |
| <a id="opt-echDecryptOnlyKeys" href="#opt-echDecryptOnlyKeys" title="#opt-echDecryptOnlyKeys">`echDecryptOnlyKeys`</a> | List of names of the referenced Kubernetes [Secrets](https://kubernetes.io/docs/concepts/configuration/secret/) (in TLSOption namespace) storing rotated-out ECH keys, still accepted for decryption but no longer advertised to clients.<br />Requires at least one entry in `echKeys`.                                                                                                                  |                            | No       |
