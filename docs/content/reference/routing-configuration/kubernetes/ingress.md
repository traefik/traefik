...
| <a id="opt-traefik-ingress-kubernetes-ioservice-strategy" href="#opt-traefik-ingress-kubernetes-ioservice-strategy" title="#opt-traefik-ingress-kubernetes-ioservice-strategy">`traefik.ingress.kubernetes.io/service.strategy`</a> | Defines the load balancing strategy between the servers.<br/>Supported values: `wrr` (weighted round-robin, default), `p2c` (power of two choices), `hrw` (highest random weight), `leasttime`.<br/>Default: `wrr` | `p2c` |
