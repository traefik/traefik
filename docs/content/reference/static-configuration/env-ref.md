...
`TRAEFIK_ENTRYPOINTS_<NAME>_EXPLICITUSEONLY`:  
Flags this EntryPoint to be skipped by default when placing a router configuration that does not specify any entrypoints. (Default: ```false```)
