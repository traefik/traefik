...
!!! note "Stdio logs are not enabled by default alongside OTLP exports"
    If you would like Stdio access logs to be available, use [accessLog.force](#opt-accesslog-force) option.
...
| <a id="opt-accesslog-force" href="#opt-accesslog-force" title="#opt-accesslog-force">`accesslog.force`</a>                                                                                     | Force Stdio logging, even if OTLP is configured. By default, Stdio logging is disabled when OTLP is enabled for performance reasons.                                                                                                                                                                                                                                   | false    | No       |
