...
| <a id="opt-OriginStatus" href="#opt-OriginStatus" title="#opt-OriginStatus">`OriginStatus`</a> | The HTTP status code resulting from proxying the request to a server: either the one returned by that server, or the one generated by the proxy error handler when the server could not be reached (`502`, `504`, ...). It is `0` when the request was never proxied to a server (e.g. a redirection). See [Origin vs. Downstream Status](../../../observe/logs-and-access-logs.md#origin-vs-downstream-status). |
...
| <a id="opt-DownstreamStatus" href="#opt-DownstreamStatus" title="#opt-DownstreamStatus">`DownstreamStatus`</a> | The HTTP status code sent **to the client**, after the middleware chain has run. This is the status the client actually observes, and the one the access log filters (`statusCodes`) and the CLF formats use. |
