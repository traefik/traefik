...
!!! note Self-Signed Default TLS Certificate
    - **When Strict SNI Checking is disabled (default setting)**: If Traefik cannot find a matching TLS certificate for a requested server name, it will automatically serve a self-signed default TLS certificate. This default behavior ensures that a certificate is always provided, even if it's not explicitly configured for the requested hostname.

    - **When Strict SNI Checking is enabled**: Traefik will only serve a certificate if it precisely matches the requested server name (SNI). If no matching certificate is available, Traefik will reject the connection instead of providing a self-signed certificate.
