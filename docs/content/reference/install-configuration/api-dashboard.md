...
| <a id="opt-api-dashboardName" href="#opt-api-dashboardName" title="#opt-api-dashboardName">`api.dashboardName`</a> | Optional label rendered as a badge in the dashboard sidebar to identify the Traefik instance (e.g. `ext`, `int`, `edge-1`). Useful when operating multiple Traefik instances whose dashboards would otherwise be visually identical. Empty string hides the badge. | "" | No |
| <a id="opt-api-dashboardNamePosition" href="#opt-api-dashboardNamePosition" title="#opt-api-dashboardNamePosition">`api.dashboardNamePosition`</a> | Controls placement of the `api.dashboardName` badge. `side` renders it next to the Traefik logo (good for short codes). `below` renders it on a new line beneath the logo (good for longer names that would crowd the version label). Has no effect when `api.dashboardName` is empty. | "side" | No |
