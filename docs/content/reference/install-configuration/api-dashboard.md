...
| <a id="opt-api-dashboardName" href="#opt-api-dashboardName" title="#opt-api-dashboardName">`api.dashboardName`</a> | Optional label rendered as a badge beneath the Traefik logo in the dashboard sidebar to identify the Traefik instance (e.g. `ext`, `int`, `edge-1`). Useful when operating multiple Traefik instances whose dashboards would otherwise be visually identical. Empty string hides the badge. | "" | No |
