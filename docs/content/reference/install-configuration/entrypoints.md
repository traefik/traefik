...
| <a id="opt-http-mergeSlashes" href="#opt-http-mergeSlashes" title="#opt-http-mergeSlashes">`http.mergeSlashes`</a> | Defines whether to merge consecutive slashes in request paths. Only applies when `sanitizePath` is enabled.<br /> More information [here](#mergeslashes).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | true                    | No       |
...
The `mergeSlashes` option defines whether consecutive slashes in request paths are merged into a single slash.
This option only applies when `sanitizePath` is enabled.

By default, consecutive slashes are merged. Setting `mergeSlashes` to `false` preserves empty path segments
while still resolving dot segments (`.` and `..`), which is useful when consecutive slashes carry semantic meaning
in multi-proxy deployments.

#### Examples

| SanitizePath | MergeSlashes | Request Path | Resulting Request Path |
|--------------|--------------|--------------|------------------------|
| <a id="opt-true-7" href="#opt-true-7" title="#opt-true-7">true</a> | true         | /foo/bar//   | /foo/bar/              |
| <a id="opt-true-8" href="#opt-true-8" title="#opt-true-8">true</a> | false        | /foo/bar//   | /foo/bar//             |
| <a id="opt-true-9" href="#opt-true-9" title="#opt-true-9">true</a> | false        | /foo/..//bar | //bar                  |
