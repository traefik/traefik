...
| <a id="opt-http-redirectSanitizedPath" href="#opt-http-redirectSanitizedPath" title="#opt-http-redirectSanitizedPath">`http.redirectSanitizedPath`</a> | Defines whether requests requiring path sanitization should receive a 301/308 redirect to the sanitized path instead of silently rewriting it.<br /> More information [here](#redirectsanitizedpath).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | false                    | No       |
...
#### RedirectSanitizedPath

_Optional, Default=false_

The `redirectSanitizedPath` option defines whether requests requiring path sanitization should receive a 301 redirect to the sanitized path instead of silently rewriting it.
This is useful when you want to ensure clients update their URLs to the canonical form.

When enabled:

- GET requests receive a `301 Moved Permanently` redirect
- Other methods (POST, PUT, etc.) receive a `308 Permanent Redirect` to preserve the request method

!!! note "Requires sanitizePath"

    This option only has effect when `sanitizePath` is enabled (the default).
