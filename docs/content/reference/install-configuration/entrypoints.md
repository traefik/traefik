...
| <a id="opt-http-tls-options" href="#opt-http-tls-options" title="#opt-http-tls-options">`http.tls.options`</a> | Apply TLS options on every router attached to the `entryPoint`. <br /> These options apply only while the router does not define its own `tls` section. <br /> If a router sets any `tls` field (for example `certResolver`), the entrypoint TLS configuration is replaced for that router (no merge). <br /> More information in the [dedicated section](../../reference/routing-configuration/http/tls/tls-options.md) and [HTTP TLS overview](../../reference/routing-configuration/http/tls/overview.md). | -                                                                  | No       |
| <a id="opt-http-tls-certResolver" href="#opt-http-tls-certResolver" title="#opt-http-tls-certResolver">`http.tls.certResolver`</a> | Apply a certificate resolver on every router attached to the `entryPoint`. <br /> Like `http.tls.options`, this applies only while the router does not define its own `tls` section. <br /> Defining any router `tls` field replaces the entrypoint TLS configuration for that router (no merge). <br /> More information in the [dedicated section](./tls/certificate-resolvers/overview.md) and [HTTP TLS overview](../../reference/routing-configuration/http/tls/overview.md). | -                                                                  | No       |
