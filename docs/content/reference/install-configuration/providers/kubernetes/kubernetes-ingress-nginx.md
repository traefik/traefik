...
| <a id="opt-providers-kubernetesIngressNGINX-proxyNextUpstream" href="#opt-providers-kubernetesIngressNGINX-proxyNextUpstream" title="#opt-providers-kubernetesIngressNGINX-proxyNextUpstream">`providers.`<br/>`kubernetesIngressNGINX.`<br/>`proxyNextUpstream`</a> | Defines in which cases a request should be retried. Accepted values are a space-separated list of: `error`, `timeout`, `http_XXX` (e.g. http_502), `non_idempotent`, and `off` (disables retry). This is used as the global proxy-next-upstream configuration when no ingress-specific value is configured. An ingress-specific configuration can be set using [`nginx.ingress.kubernetes.io/proxy-next-upstream`](../../../routing-configuration/kubernetes/ingress-nginx.md#opt-nginx-ingress-kubernetes-ioproxy-next-upstream) annotation.                  | "error timeout" | No       |
| <a id="opt-providers-kubernetesIngressNGINX-proxyNextUpstreamTries" href="#opt-providers-kubernetesIngressNGINX-proxyNextUpstreamTries" title="#opt-providers-kubernetesIngressNGINX-proxyNextUpstreamTries">`providers.`<br/>`kubernetesIngressNGINX.`<br/>`proxyNextUpstreamTries`</a> | Limits the number of possible tries if the backend server does not reply. 0 means unlimited tries, which is capped to the number of available servers. This is used as the global retry count configuration when no ingress-specific value is configured. An ingress-specific retry limit can be set using [`nginx.ingress.kubernetes.io/proxy-next-upstream-tries`](../../../routing-configuration/kubernetes/ingress-nginx.md#opt-nginx-ingress-kubernetes-ioproxy-next-upstream-tries) annotation.                                                          | 3               | No       |
...
| <a id="opt-providers-kubernetesIngressNGINX-disableNonTLSRouters" href="#opt-providers-kubernetesIngressNGINX-disableNonTLSRouters" title="#opt-providers-kubernetesIngressNGINX-disableNonTLSRouters">`providers.`<br/>`kubernetesIngressNGINX.`<br/>`disableNonTLSRouters`</a> | Disables the creation of non-TLS routers for Ingress resources. Cannot be used together with `httpEntryPoint`.<br />More information [here](#disablenontlsrouters).                                                                                                                                                                                                                                                                                                                                                                                                                                  | false  | No       |
...
### `disableNonTLSRouters`

Disables the creation of non-TLS routers for Ingress resources managed by this provider.
The HTTP entryPoint itself is unaffected and keeps serving other providers.

!!! note

    When `disableNonTLSRouters` is `true`, also set `httpsEntryPoint` if you want Ingress traffic served only on the HTTPS entryPoint.
    Otherwise the TLS routers may still bind to the default entryPoints (including non-TLS ones such as `web`).
