...
| `providers.`<br/>`kubernetesIngressNGINX.`<br/>`proxyNextUpstream`</a>                                                                                                                                                                                                                           | Defines in which cases a request should be retried. There is a possibility that the retry request would be passed to the same server. This is used as the global proxy-next-upstream configuration when no ingress-specific value is configured. An ingress-specific configuration can be set using [`nginx.ingress.kubernetes.io/proxy-next-upstream`](../../../../routing-configuration/kubernetes/ingress-nginx/#opt-nginx-ingress-kubernetes-ioproxy-next-upstream) annotation.                   | "error timeout" | No       |
| `providers.`<br/>`kubernetesIngressNGINX.`<br/>`proxyNextUpstreamTries`</a>                                                                                                                                                                                                                      | Limits the number of possible tries if the backend server does not reply. 0 means unlimited tries, which is capped to the number of available server. This is used as the global retry count configuration when no ingress-specific value is configured. An ingress-specific retry limit can be set using [`nginx.ingress.kubernetes.io/proxy-next-upstream-tries`](../../../../routing-configuration/kubernetes/ingress-nginx/#opt-nginx-ingress-kubernetes-ioproxy-next-upstream-tries) annotation. | 3               | No       |
