...
2. Install/update the Baqup [RBAC](../../../dynamic-configuration/kubernetes-gateway-rbac.yml).
...
When deployed into Kubernetes, Baqup reads the environment variables
...
When the environment variables are not found, Baqup tries to connect to
...
!!! tip "Routing Configuration"
    ...
    When using the Kubernetes Gateway API provider, Baqup uses the Gateway API
