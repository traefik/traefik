...
It allows multiple Traefik instances pointed at the same Swarm endpoint
...
??? example "routerLabels + per-router constraint Examples"
    ...
    [providers.swarm.routerLabels]
      tier = "ext"
    ```

    ```bash tab="CLI — Traefik static config"
    --providers.swarm.routerLabels.tier=ext
    ```

    ```yaml tab="Swarm service labels (dynamic config)"
