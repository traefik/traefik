...
| <a id="opt-providers-consulCatalog-allowEmptyServices" href="#opt-providers-consulCatalog-allowEmptyServices" title="#opt-providers-consulCatalog-allowEmptyServices">`providers.consulCatalog.allowEmptyServices`</a> | Allows creating a route to reach a service that has no healthy instance available according to `strictChecks`.<br />It allows Traefik to handle requests targeting this service before returning a `503` HTTP status. | false | No |
