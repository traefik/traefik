...
You can also optionally configure the `passSSLClientCert` option to pass the Client certificates to the backend in a specific header.
...
[frontends]
  ...
  backend = "backend1"
  passHostHeader = true
  [frontends.frontend2.passSSLClientCert]
    pem = true
  priority = 10
