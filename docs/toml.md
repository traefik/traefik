...
# Access logs are specified lower down.
#
# Optional
#
...
The standard access log uses the textual Common Log Format (CLF), extended
with some more fields. An alternative is to write JSON, which is described later.
...
Access logs are by default written using a one-line buffer. You can increase
the buffering, e.g.
```
[accessLog]
  file       = "access.log"
...
If you need logs in JSON, set `format = "json"` and, optionally, define the four blocks
of field mappings, which are
...
[accessLog]
  ...
  format = "json"
  coreFields = ["HTTPMethod", "HTTPRequestPath", "HTTPProtocol"]
  requestHeaders = ["Host"]
  originResponseHeaders = ["Content-Type", "Server"]
  downstreamResponseHeaders = ["Location", "Content-Length"]

```
JSON records generated by the above will include "HTTPMethod", "HTTPRequestPath", "HTTPProtocol"
 ...
 * HTTPAddr: the requested host:IP (as per 'Host' header);
   HTTPHost and HTTPPort are extracted from this address.

 * HTTPMethod: the request method.

 * HTTPRequestPath: the request path including query parameters, fragment identifier etc

 * HTTPProtocol: the HTTP protocol (e.g. "HTTP/1.1")

 * HTTPRequestLine: the original HTTP line, which consists of the method, the request path and the protocol

 * ClientRemoteAddr: the remote address of the user-agent;
 ...
 * OriginDuration: time taken by the origin server ('upstream')
   to return its response. Omitted if the information is unavailable.
...
[accessLog]
  ...
  coreFields = [
    ...
    "HTTPAddr:              host",
    "HTTPMethod:            method",
    "HTTPRequestPath:       request_path",
    "HTTPProtocol:          protocol",
    "ClientRemoteAddr:      remote_addr",
