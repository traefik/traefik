...
The standard access log uses the textual Common Log Format (CLF), extended with some more fields.
An alternative is to write JSON, which is described later.
...
The file can be gzipped as it is written if you require; just add a `.gz` extension:
...
The timestamps written to the log file are in the CLF pattern (`02/Jan/2006:15:04:05 -0700`).
This can be overridden using the `timeFormat` setting, e.g.
...
Access logs are by default written using a one-line buffer.
You can increase the buffering, e.g.
...
which improves the write performance but there is a slight delay before some items are written.
Note however that the buffer size is ignored if you have a gzipped output file (gzipping also implies buffering of its own).

### Access logs in JSON

If you need logs in JSON, set `format = "json"` and, optionally, define the four blocks of field mappings, which are
...
When the output format is `json`, these four select which items are included (otherwise they are ignored).
...
[accessLog]
  ...
  format = "json"
  coreFields = ["RequestMethod", "RequestPath", "RequestProtocol"]
  requestHeaders = ["Host"]
  originResponseHeaders = ["Content-Type", "Server"]
  downstreamResponseHeaders = ["Location", "Content-Length"]
```
...
Fields are only written to the output when the corresponding source data is present.
Absent data is silently omitted from the output.

Note in particular that the origin header values will be absent for requests that never reached an origin server (i.e. redirects).
Also note that the internal middleware can alter response headers,
so the origin and downstream response headers might differ (e.g. for Content-Length).

Also note that multi-value headers are not fully reported - only the first value is used.

The order of these fields determines the output ordering (although this is JSON so order is not very important).
...
* a source key name and an output key name, separated by colon.
  This allows fields to be renamed as required, e.g. `StartUTC:time_utc`. There are more examples below.
...
* `OriginStatus`: the status code returned from the origin server.
  `OriginStatusLine` contains the code and corresponding descriptive string.

* `OriginContentSize`: the number of bytes in the response entity.

* `DownstreamStatus`: the status code sent back to the client.
  `DownstreamStatusLine` contains the code and corresponding descriptive string.

* `DownstreamContentSize`: the number of bytes sent back in the response entity.

* `FrontendName`: the name of Traefik's frontend.

* `BackendName`: the name of Traefik's backend.

* `BackendURL`: the URL for the backend.

* `StartUTC`: the UTC time when the request was received.

* `StartLocal`: the local time when the request was received.

* `Duration`: the total time taken by processing the response,
  including the origin server's time but not the log writing time.
  Always present.

* `OriginDuration`: time taken by the origin server ('upstream') to return its response.
  Omitted if the information is unavailable.

* `Overhead`: the processing time overhead caused by Traefik, which
  is the difference between the above two.

* `GzipRatio`: the response body compression ratio achieved. A
  number less than 1 indicates some compression. Note that very
  small responses are typically made larger by compression, in
  which case the `GzipRatio` might be greater than 1.
  Omitted if the information is unavailable.
