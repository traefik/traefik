...
Træfik can be configured to use snapd as a provider.
...
# Enable Snapd Provider.
[snapd]

# Snapd server endpoint. Can be a tcp or a unix socket endpoint.
#
# Optional
#
endpoint = "unix:///run/snapd.socket"
...
# Required
#
domain = "snapd.localhost"

# Enable watch snapd changes.
#
# Optional
#
watch = true
...
## Properties: overriding default behavior

Properties can be set using `snap set <snap> <property>=<value> on snaps to override default behavior.
...
| `traefik.frontend.auth.basic=EXPR`                                  | Sets the basic authentication to this frontend in CSV format: `User:Hash,User:Hash` [1] (DEPRECATED).                                                                                                                            |
...
| `traefik.frontend.auth.basic.users=EXPR`                            | Sets the basic authentication to this frontend in CSV format: `User:Hash,User:Hash` [1].                                                                                                                                         |
...
| `traefik.frontend.rule=EXPR`                                        | Overrides the default frontend rule. Default: `Host:{snapName}.{domain}`.                                                                     |
...
[1] `traefik.frontend.auth.basic.users=EXPR `:
...
| Label                                                 | Description                                                                                                                                                               |
|-------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `traefik.frontend.headers.customRequestHeaders=EXPR ` | Provides the snap with custom request headers that will be appended to each request forwarded to the snap.<br>Format: <code>HEADER:value&vert;&vert;HEADER2:value2</code> |
| `traefik.frontend.headers.customResponseHeaders=EXPR` | Appends the headers to each response returned by the snap, before forwarding the response to the client.<br>Format: <code>HEADER:value&vert;&vert;HEADER2:value2</code>   |
...
## On snaps with Multiple Ports (segment properties)

Segment properties are used to define routes to a snap exposing multiple ports.
A segment is a group of properties that apply to a port exposed by a snap.
You can define as many segments as ports exposed in a snap.

Segment properties override the default behavior.
...
!!! note
    If a label is defined both as a `snap property` and a `segment property` (for example `traefik.<segment_name>.port=PORT` and `traefik.port=PORT` ), the `segment property` is used to defined the `<segment_name>` property (`port` in the example).

    It's possible to mix `snap properties` and `segment properties`, in this case `snap properties` are used as default value for missing `segment properties` but no frontends are going to be created with the `snap properties`.
