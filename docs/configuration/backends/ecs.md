...
| `traefik.containerPort=80`                                 | Specifies the exposed port in a container found in `NetworkBindings`. Routes to the host port mapped to this value. Good for services with multiple exposed, dynamically mapped ports. Precedence is given to `traefik.port`.
