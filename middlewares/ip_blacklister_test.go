...
func TestNewIPBlackLister(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedError:    "parsing CIDR blacklist [foo]: parsing CIDR white list <nil>: invalid CIDR address: foo",
		},
		{
			desc:             "valid IP",
			blackList:        []string{"10.10.10.10"},
			useXForwardedFor: false,
			expectedError:    "",
		},
	}
	...
	for _, test := range testCases {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			blackLister, err := NewIPBlackLister(test.blackList, test.useXForwardedFor)
...
func TestIPBlackLister_ServeHTTP(t *testing.T) {
	...
	for _, test := range testCases {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			blackLister.ServeHTTP(recorder, req, next)
