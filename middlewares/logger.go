...
func (l *Logger) ServeHTTP(rw http.ResponseWriter, r *http.Request, next http.HandlerFunc) {
	if l.file == nil {
	   	fmt.Printf("debug...(l *Logger) ServeHTTP: l.file was zero; calling next(rw, r)\n")
		next(rw, r)
	} else {
	   	fmt.Printf("debug...(l *Logger) ServeHTTP: l.file was not zero: %v\n", l.file)

//		CombinedLoggingHandler(l.file, next).ServeHTTP(rw, r)
//		fmt.Fprintf(l.file, "debug... about to call combinedlogginghandler...\n")
//		temp := CombinedLoggingHandler(l.file, next)
//		fmt.Fprintf(l.file, "debug... about to call ServeHTTP\n")
//		temp.ServeHTTP(rw, r)
//		fmt.Fprintf(l.file, "debug... returned from ServeHTTP\n")
		start := time.Now()
		next(rw, r)
		createLogEntry(l.file, rw, r, start)
		//fmt.Fprintf(l.file, "debug... request: %+w\n", r)
		//fmt.Fprintf(l.file, "\ndebug... response:\n")
		//spew.Fdump(l.file, rw)
		//fmt.Fprintf(l.file, "\n\n")
	}
}
...
func createLogEntry(l *os.File, resp http.ResponseWriter, req *http.Request, start time.Time) {
        url := *req.URL
	username := "-"
	if url.User != nil {
	...
	len := respHeader["Content-Length"][0]
	elapsed := time.Now().UTC().Sub(start.UTC())
	// TODO The gorlla/handlers/handlers.go wrote the HTTP Response to get the status and length.
	//      Do we want to do the same?
	// TODO The backend host is currently displayed as req.RemoteAddr, but this is not correct.
	//      A new middleware layer is required to hold the necessary context information.
	fmt.Fprintf(l, `%s - %s [%s] "%s" "%s" "%s" %s "%s" "%s" "%s" "%s" %s %s`,
	    host, username, ts, req.Method, uri, req.Proto, len, req.Referer(), req.UserAgent(), req.Host, req.RemoteAddr, elapsed, "\n")
}



// From github.com/gorilla/handlers
...
func (h combinedLoggingHandler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	writeCombinedLog(h.writer, req, url, t, logger.Status(), logger.Size(), elapsed)
}
...
func buildCommonLogLine(req *http.Request, url url.URL, ts time.Time, status int, size int) []byte {
	...
	buf := make([]byte, 0, 3*(len(host)+len(username)+len(req.Method)+len(uri)+len(req.Proto)+100)/2)
	buf = append(buf, host...)
	buf = append(buf, " - "...)
...
// ts is the timestamp with which the entry should be logged.
// status and size are used to provide the response HTTP status and size.
func writeCombinedLog(w io.Writer, req *http.Request, url url.URL, ts time.Time, status, size int, elapsed time.Duration) {
	buf := buildCommonLogLine(req, url, ts, status, size)
	buf = append(buf, ` "`...)
	...
	buf = append(buf, `" "`...)
	buf = appendQuoted(buf, req.RemoteAddr)
	buf = append(buf, `" "`...)
	buf = append(buf, fmt.Sprintf("%v", elapsed)...)
	buf = append(buf, '"', '\n')
	w.Write(buf)
}
