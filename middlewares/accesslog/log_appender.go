...
func NewLogAppender(settings *types.AccessLog) (LogAppender, []error) {
	...
	case "json":
		var errs []error
		formatter, errs = newJSONLogFormatter(settings)
		if errs != nil {
			return LogAppender{}, errs
		}
...
func (l *LogAppender) openLogFileAndBuffer() error {
	...
	if l.settings.BufferSize != "" {
		if bufferSize, _, err = types.AsSI(l.settings.BufferSize); err != nil {
			return err
		}
	}

	log.Debugf("Opening access log %s (%s)", l.settings.File, l.settings.Format)
	if l.file, err = os.OpenFile(l.settings.File, os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0644); err != nil {
		return err
	}
...
func (l *LogAppender) Open() error {
	if len(l.settings.File) > 0 && !l.IsOpen() {
		if err := l.openLogFileAndBuffer(); err != nil {
			return err
		}
...
func (l *LogAppender) Close() error {
	if l.IsOpen() {
		if err := l.flushBufferAndCloseFile(); err != nil {
			return err
		}
	}
	return nil
