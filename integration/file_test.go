...
func (s *FileSuite) TestMultipleRouteConfiguration(c *check.C) {
	cmd, display := s.traefikCmd(withConfigFile("fixtures/file/multiple_route.toml"))
	defer display(c)
	err := cmd.Start()
	c.Assert(err, checker.IsNil)
	defer cmd.Process.Kill()
	...
	err = try.GetRequest("http://127.0.0.1:8000/test1", 1000*time.Millisecond, try.StatusCodeIs(http.StatusBadGateway))
	c.Assert(err, checker.IsNil)
