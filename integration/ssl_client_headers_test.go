...
type SSLClientHeadersSuite struct{ BaseSuite }

func (s *SSLClientHeadersSuite) SetUpSuite(c *check.C) {
	s.createComposeProject(c, "sslclientheaders")
	s.composeProject.Start(c)
}

func (s *SSLClientHeadersSuite) TestSSLClientHeaders(c *check.C) {
	rootCertContent, err := ioutil.ReadFile("./fixtures/sslclientheaders/root.pem")
	c.Assert(err, check.IsNil)
	serverCertContent, err := ioutil.ReadFile("./fixtures/sslclientheaders/server.pem")
	c.Assert(err, check.IsNil)
	ServerKeyContent, err := ioutil.ReadFile("./fixtures/sslclientheaders/server.key")
	c.Assert(err, check.IsNil)

	file := s.adaptFile(c, "fixtures/sslclientheaders/simple.toml", struct {
	...
	certificate, err := tls.LoadX509KeyPair("./fixtures/sslclientheaders/server.pem", "./fixtures/sslclientheaders/server.key")
	c.Assert(err, checker.IsNil)
