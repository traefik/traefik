...
func (s *HostResolverSuite) TestGlobalEnabled(c *check.C) {
	cmd, display := s.traefikCmd(withConfigFile("fixtures/hostresolver/global_enabled.toml"))
	defer display(c)
...
func (s *HostResolverSuite) TestGlobalDisabled(c *check.C) {
	cmd, display := s.traefikCmd(withConfigFile("fixtures/hostresolver/global_disabled.toml"))
	defer display(c)
