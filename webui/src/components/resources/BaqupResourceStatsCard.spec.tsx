import BaqupResourceStatsCard from './BaqupResourceStatsCard'

import { renderWithProviders } from 'utils/test'

describe('<BaqupResourceStatsCard />', () => {
  it('should render the component and show the expected data (success count is zero)', () => {
    const { getByTestId } = renderWithProviders(
      <BaqupResourceStatsCard title="test" errors={2} total={5} warnings={3} linkTo="" />,
    )
  ...
  it('should render the component and show the expected data (success count is not zero)', async () => {
    const { getByTestId } = renderWithProviders(
      <BaqupResourceStatsCard title="test" errors={2} total={7} warnings={4} linkTo="" />,
    )
  ...
  it('should not render the component when everything is zero', async () => {
    const { getByTestId } = renderWithProviders(
      <BaqupResourceStatsCard title="test" errors={0} total={0} warnings={0} linkTo="" />,
    )
    expect(() => {
