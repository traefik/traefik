...
{{ $backend := index $servers 0 }}
  ...
  {{ $circuitBreaker := getCircuitBreaker $backend.RoadLabels }}
  {{if $circuitBreaker }}
  [backends."backend-{{ $backendName }}".circuitBreaker]
    expression = "{{ $circuitBreaker.Expression }}"
  {{end}}

  {{ $loadBalancer := getLoadBalancer $backend.RoadLabels }}
  {{if $loadBalancer }}
    [backends."backend-{{ $backendName }}".loadBalancer]
      method = "{{ $loadBalancer.Method }}"
      sticky = {{ $loadBalancer.Sticky }}
  ...
  {{ $maxConn := getMaxConn $backend.RoadLabels }}
  {{if $maxConn }}
  [backends."backend-{{ $backendName }}".maxConn]
    extractorFunc = "{{ $maxConn.ExtractorFunc }}"
    amount = {{ $maxConn.Amount }}
  ...
  {{ $healthCheck := getHealthCheck $backend.RoadLabels }}
  {{if $healthCheck }}
  [backends."backend-{{ $backendName }}".healthCheck]
    path = "{{ $healthCheck.Path }}"
    port = {{ $healthCheck.Port }}
  ...
  {{ $buffering := getBuffering $backend.RoadLabels }}
  {{if $buffering }}
  [backends."backend-{{ $backendName }}".buffering]
    maxRequestBodyBytes = {{ $buffering.MaxRequestBodyBytes }}
    memRequestBodyBytes = {{ $buffering.MemRequestBodyBytes }}
...
{{range $frontendName, $containers := .Frontends }}
  ...
  [frontends."frontend-{{ $frontendName }}"]
    ...
    priority = {{ getPriority $container.RoadLabels }}
    passHostHeader = {{ getPassHostHeader $container.RoadLabels }}
    passTLSCert = {{ getPassTLSCert $container.RoadLabels }}

    entryPoints = [{{range getEntryPoints $container.RoadLabels }}
      "{{.}}",
      {{end}}]

    {{ $whitelistSourceRange := getWhitelistSourceRange $container.RoadLabels }}
    {{if $whitelistSourceRange }}
    whitelistSourceRange = [{{range $whitelistSourceRange }}
      "{{.}}",
      {{end}}]
    ...
    basicAuth = [{{range getBasicAuth $container.RoadLabels }}
      "{{.}}",
      {{end}}]

    {{ $redirect := getRedirect $container.RoadLabels }}
    {{if $redirect }}
    ...
    {{ $errorPages := getErrorPages $container.RoadLabels }}
    {{if $errorPages }}
    ...
    {{ $rateLimit := getRateLimit $container.RoadLabels }}
    {{if $rateLimit }}
    ...
    {{ $headers := getHeaders $container.RoadLabels }}
    {{if $headers }}
